...
class TiedCreator extends ConsumerStatefulWidget {
  ...
  const TiedCreator({super.key, required this.id});

  @override
  ConsumerState<TiedCreator> createState() => _TiedCreator();
}

class _TiedCreator extends ConsumerState<TiedCreator> {
  bool _loading = false;
  ...
  Widget build(BuildContext context) {
    ...
    final winnerOptions = sortedOptions.where((option) => (option.num_votes ?? 0) == winnerValue).toList();
            ...
            children: [
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemCount: winnerOptions.length,
                  itemBuilder: (context, index) {
                    final option = winnerOptions[index];
                            ...
                            try {
                              ...
                              if(decision.type == DecisionType.ranking) {
                                  await ref.read(createProvider.notifier).createRankingVote(
                                    id_option: _selectedOptionId!, 
                                    id_decision: decision.id, 
                                    id_user: currentUserId!,
                                    number: -1, //le resta uno a la opción para que sea la preferente al ser tipo ranking
                                     tied: true,
                                  );
                              } else {
                                await ref.read(createProvider.notifier).createSimpleVote(id_option: _selectedOptionId!, id_decision: decision.id, id_user: currentUserId!, tied:true);
                              }
