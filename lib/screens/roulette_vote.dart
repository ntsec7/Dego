...
class RouletteVote extends ConsumerStatefulWidget {
  final String id;

  const RouletteVote({super.key, required this.id});

  @override
  ConsumerState<RouletteVote> createState() => _RouletteVote();
}

class _RouletteVote extends ConsumerState<RouletteVote> with SingleTickerProviderStateMixin {
  bool _loading = false;
...
//Girar la ruleta
  void _spinRoulette(List<dynamic> options, String decisionId, String currentUserId) async {
    ...
    _animationController.forward(from: 0.0).then((_)  async {
      ...
      await ref.read(createProvider.notifier).createSimpleVote(id_option: _selectedOptionId!, id_decision: decisionId, id_user: currentUserId);
  ...
  Widget build(BuildContext context) {
    ...
    _initializeColors(options.length);
              ...
              ElevatedButton.icon(
                onPressed: _animationController.isAnimating || _loading ? null : () async {
                  ...
                  try {
                    ...
                    // Gira la ruleta
                    _spinRoulette(options, decision.id, currentUserId);
