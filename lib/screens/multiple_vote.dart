...
class MultipleVote extends ConsumerStatefulWidget {
  ...
  const MultipleVote({super.key, required this.id});

  @override
  ConsumerState<MultipleVote> createState() => _MultipleVote();
}

class _MultipleVote extends ConsumerState<MultipleVote> {
  bool _loading = false;
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: _loading
                        ? null
                        : () async {
                            // Validación: Verificar que haya seleccionado al menos una opción antes de votar
                            if (_selectedOptionIds.isEmpty) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(context.lang.error_votar_multiple_no_ops),
                                ),
                              );
                              return;
                            }
                            ...
                            try {
                              ...
                              //Vota
                              for (String optionId in _selectedOptionIds) {
                                await ref.read(createProvider.notifier).createRankingVote(
                                  id_option: optionId, 
                                  id_decision: decision.id, 
                                  id_user: currentUserId,
                                  number: 1,  //el voto siempre vale 1 punto
                                );
                              }
